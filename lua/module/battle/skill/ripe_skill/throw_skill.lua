...
function this:update_by_straight(sess,delta )
    ...
    local de_x = self.target_pos.X - self.start_pos.X
    local de_y = self.target_pos.Y - self.start_pos.Y
    local de_z = 0.5 - self.start_pos.Z
    local dis = math.sqrt( de_x*de_x,de_y*de_y )
    local time = dis/self.speed

    if dis < 2 then  return "completed" end
    ...
    self.start_pos.X = self.start_pos.X + de_x
    self.start_pos.Y = self.start_pos.Y + de_y
    self.start_pos.Z = self.start_pos.Z + de_z

    self.effect_entity:SetPos(self.start_pos.X,self.start_pos.Y,self.start_pos.Z)
